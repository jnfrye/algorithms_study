...
void MaxHeapIncreaseKey(std::vector<int> &max_heap, const int node, const int new_key) 
{
	...
	while (current_node > 0 && max_heap[Parent(current_node)] < new_key) 
	{
...
{
	assert (new_key < min_heap[node] && "New key must be smaller than old key!");

	min_heap[node] = new_key;
	int current_node = node;
	while (current_node > 0 && min_heap[Parent(current_node)] > new_key) 
	{
