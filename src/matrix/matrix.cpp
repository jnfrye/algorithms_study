			...
			for (int subrow = 0; subrow < row_dims[row]; ++subrow)
			{
				for (int subcol = 0; subcol < col_dims[col]; ++subcol)
				{
					split_matrix[row][col][subrow][subcol]
						= A[subrow + row * row_dims[0]][subcol + col * col_dims[0]];
				}
			}
		}
	}
...
Matrix MatrixMultiplyDAC(const Matrix &left, const Matrix &right)
{
	...
	if (num_rows == 1 || left[0].size() == 1 || right.size() == 1 || num_cols == 1)
	{
		...
		for (int row = 0; row < 2; ++row)
		{
			for (int col = 0; col < 2; ++col)
			{
				split_product[row][col] =
					MatrixAdd(
						MatrixMultiplyDAC(split_left[row][0], split_right[0][col]),
						MatrixMultiplyDAC(split_left[row][1], split_right[1][col]));
			}
		}
...
Matrix MatrixMultiplyStrassen(const Matrix &left, const Matrix &right)
{
	...
	else
	{
		...
		products[0] = MatrixMultiplyStrassen(
			split_left[0][0], MatrixSubtract(split_right[0][1], split_right[1][1]));
		products[1] = MatrixMultiplyStrassen(
			MatrixAdd(split_left[0][0], split_left[0][1]), split_right[1][1]);
		products[2] = MatrixMultiplyStrassen(
			MatrixAdd(split_left[1][0], split_left[1][1]), split_right[0][0]);
		products[3] = MatrixMultiplyStrassen(
			split_left[1][1], MatrixSubtract(split_right[1][0], split_right[0][0]));
		...
		split_result[0][0] = MatrixAdd(
			products[4], MatrixAdd(products[5], MatrixSubtract(products[3], products[1])));
		...
		split_result[1][1] = MatrixAdd(
			products[4],
			MatrixSubtract(MatrixSubtract(products[0], products[2]), products[6]));
